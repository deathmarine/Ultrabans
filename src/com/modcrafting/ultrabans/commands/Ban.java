...
public class Ban extends CommandHandler {
	...
	public String command(CommandSender sender, Command command, String[] args) {
		...
		if (plugin.cache.containsKey(uuid)) {
			for (InfoBan info : plugin.cache.get(uuid)) {
		...
		if (victim != null) {
			if (victim.isOnline()) {
				...
				victim.getPlayer().kickPlayer(
						ChatColor.translateAlternateColorCodes('&', vicmsg));
			}
			name = victim.getName();
		}

		plugin.getAPI().banPlayer(uuid, reason, admin);

		String bcmsg = ChatColor.translateAlternateColorCodes('&',
				lang.getString("Ban.MsgToBroadcast"));
		...
		// if(config.getBoolean("CleanOnBan")) Formatting.deletePlyrdat(name);
